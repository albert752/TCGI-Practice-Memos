...
### Section 2
Now we need to configure Alice's machine and Carla's to VLAN10 and the other
stations to untagged. To do so we only need to configure L3 and L1 with the
following commands on L3:
...
root@L3:~# ifconfig eth1 up
root@L3:~# ifconfig br3-10 10.0.0.32/24
```
...
And then we run the following command the on L1:

```
root@L1:~#brctl delif br1 eth0
br1: port 1(eth0) entered disabled state
...
So we achieve this configuration on L1:

```
root@L1:~# brctl show
bridge name     bridge id               STP enabled     interfaces
br1             8000.fefd00000701       no              eth1
                                                        eth2
br1-10          8000.fefd00000700       no              eth0
                                                        eth1.10
```

With this configuration, we have created a trunk between L1 and L3. 

We test our set-up by sending a broadcast frame from Carla while capturion on
SimNet1 and SimNet2. 

![Image 8](./images/scrot_8.png)

As expected it is only captured on SimNet1. 

Now we send a broadcast frame from Frank. Then again as expected, it gets
captured on SimNet1 and SimNet2.
